...
/**
 * Tomcat 设计了一系列抽象基类来封装 稳定不变的部分，抽象基类 AbstractProtocol 实现了 ProtocolHandler 接口。
 *
    ...
    protected static class ConnectionHandler<S> implements AbstractEndpoint.Handler<S> {
        ...
        public SocketState process(SocketWrapperBase<S> wrapper, SocketEvent status) {
            ...
            try {
                ...
                if (processor == null) {
                    processor = recycledProcessors.pop();// processor=org.apache.coyote.http11.Http11Processor
                    if (getLog().isDebugEnabled()) {
                ...
                connections.put(socket, processor); // 存入 processor

                SocketState state = SocketState.CLOSED;
                do {
                    state = processor.process(wrapper, status);// Processor进行处理
