 ...
 * 线程模型: 主从reactor
 * <ul>
 * <li>Socket acceptor thread</li> 监听链接
 * <li>Socket poller thread</li> IO操作
...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel> {
     ...
     */
    private volatile ServerSocketChannel serverSock = null; // 服务端 Channel 用于接收链接
    ...
    /**
     * Poller thread count. IO线程，最少2个,正常情况下 = availableProcessors()
     */
    ...
    /**
     * The socket poller. Poller[] 代表了IO线程
     */
    ...
    /**
     * Initialize the endpoint. 需要绑定端口并监听连接
     */
    @Override
    public void bind() throws Exception {
        ...
        if (!getUseInheritedChannel()) {
            ...
            // 绑定地址端口 等待队列长度 backlog=100 (当应用层面的连接数到达最大值时，操作系统可以继续接收连接，那么操作系统能继续接收的最大连接数就是这个队列长度)
        ...
        if (pollerThreadCount <= 0) {
            //minimum one poller thread   pollerThreadCount最少2个,跟availableProcessors有关
            pollerThreadCount = 1;
        }
    ...
    /**
     * Start the NIO endpoint, creating acceptor, poller threads. 启动 NIO endpoint 创建 acceptor ,IO线程
     */
    @Override
    public void startInternal() throws Exception {
        ...
        if (!running) {
            ...
            // Create worker collection 创建工作线程池  max threads:200
            if (getExecutor() == null) {
                createExecutor();
            }
     ...
     * and processing may continue, <code>false</code> if the socket needs to be close immediately
     */
    protected boolean setSocketOptions(SocketChannel socket) {
        // Process the connection
        try {
            //disable blocking, APR style, we are gonna be polling it
            socket.configureBlocking(false);// 配置非阻塞
            Socket sock = socket.socket();
            // 配置 sock 相关属性
            socketProperties.setProperties(sock);

            // NioChannel 用于包装刚接收的客户端连接 SocketChannel
            ...
            if (channel == null) {
                ...
                // 将客户端IO的SocketChannel设置进去
                channel.setIOChannel(socket);
                channel.reset();
            }

            /**
             * 将 channel 注册到 Poller (Selector): 将NioChannel包装到封装在一个 PollerEvent 对象中，并将 PollerEvent 对象压入 Poller 的 Queue 里，
    ...
    protected class Acceptor extends AbstractEndpoint.Acceptor {
        ...
        public void run() {
            ...
            while (running) {
                ...
                try {
                    //if we have reached max connections, wait 如果达到最大连接数,应用层面则采取等待
                    ...
                    try {
                        ...
                        countDownConnection(); // 连接建立失败,释放连接数 ,-1
                        if (running) {
    ...
    public class Poller implements Runnable {
        ...
        public boolean events() {
            ...
            for (int i = 0, size = events.size(); i < size && (pe = events.poll()) != null; i++) {
                result = true;
                try {
                    pe.run(); // 注册 SelectionKey.OP_READ
                    pe.reset();
                    if (running && !paused) {
            ...
            try {
                ...
                if (handshake == 0) {
                    ...
                    if (event == null) {
                        ...
                        // 处理事件
                        state = getHandler().process(socketWrapper, event);// AbstractProtocol.ConnectionHandler
                    }
