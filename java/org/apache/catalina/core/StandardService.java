...
public class StandardService extends LifecycleMBeanBase implements Service {
    ...
    /**
     * The <code>Server</code> that owns this Service, if any. 属于那个Server
     */
    ...
    private Engine engine = null; // Engine 容器
    ...
    /**
     * Mapper listener.
     *  为什么有一个 MapperListener？
     *  这是因为 Tomcat 支持热部署，当 Web 应用的部署发生变化时，Mapper 中的映射信息也要跟着变化，
     *  MapperListener 就是一个监听器，它监听容器的变化，并把信息更新到 Mapper 中，典型的观察者模式
     */
    ...
    protected void startInternal() throws LifecycleException {
        ...
        // Start our defined Container first  启动 Engine 组件 Engine 会启动它子容器
        if (engine != null) {
            synchronized (engine) {
                engine.start(); // StandardEngine
            }
        }
        // Start Executor --> StandardThreadExecutor
        synchronized (executors) {
            for (Executor executor: executors) {
                executor.start();
            }
