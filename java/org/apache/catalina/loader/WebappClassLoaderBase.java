...
public abstract class WebappClassLoaderBase extends URLClassLoader
    ...
    protected WebappClassLoaderBase() {
        ...
        if (p == null) {
            // 默认 SystemClassLoader
            p = getSystemClassLoader();
        }
        this.parent = p;
        //log.debug("this.parent ClassLoader="+this.parent);
        ...
        //log.debug("this.javaseClassLoader ClassLoader="+this.javaseClassLoader);
    ...
    protected WebappClassLoaderBase(ClassLoader parent) {
        ...
        // get parent ClassLoader
        ClassLoader p = getParent(); // parent= SharedClassLoader
        if (p == null) {
            p = getSystemClassLoader();
        }
        this.parent = p; // parent= SharedClassLoader
        ...
        this.javaseClassLoader = j;// javaseClassLoader=ExtClassLoader
    ...
    public Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
        ...
        synchronized (getClassLoadingLock(name)) {
            ...
            // Log access to stopped class loader  检查类加载器组件状态
            checkStateForClassLoading(name);

            // (0) Check our previously loaded local class cache   检查本地缓存
            ...
            // (0.1) Check our previously loaded class cache 检查当前类加载器是否已加载过
            ...
            //       SRV.10.7.2  防止 Web 应用自己的类覆盖 JRE 的核心类
            String resourceName = binaryNameToPath(name, false);

            ClassLoader javaseLoader = getJavaseClassLoader();// Ext ClassLoader
            ...
            try {
                ...
                if (securityManager != null) {
                    ...
                    // javaseLoader 根据资源名称 获取 url 【检查自己是否能够加载】
            ...
            if (tryLoadingFromJavaseLoader) {
                try {
                    // to prevent the webapp from overriding Java SE classes
            ...
            // (0.5) Permission to access this class when using a SecurityManager 【checkPackageAccess】
            if (securityManager != null) {
                int i = name.lastIndexOf('.');
                if (i >= 0) {
                    try {
            ...
            // (1) Delegate to our parent if requested  代理给 parent Classloader【delegateLoad控制是否遵循双亲委派】  默认不走
            if (delegateLoad) {
                if (log.isDebugEnabled()) {
            ...
            // (3) Delegate to parent unconditionally 当前类加载器加载不到,委派给 parent
            if (!delegateLoad) {
                if (log.isDebugEnabled()) {
